...
`hermes-1.10.0`); bumping it and redeploying creates a new task-definition
...
# → ghcr.io/boldblackai/harness:hermes-1.10.0
```
...
HarnessImageTag:
  Type: String
  Default: hermes-1.10.0     # ← was hermes-1.9.3
```
...
aws cloudformation deploy \
  ...
  --parameter-overrides $OVERRIDES HarnessImageTag=hermes-1.10.0
```
...
Expect `Image` ending in `:hermes-1.10.0`, `Status = RUNNING`. Tail the gateway
...
- **Image upgrades are stack updates, not overlays.** Mode 4 bumps
  `HarnessImageTag` (a CloudFormation parameter, default `hermes-1.10.0`)
